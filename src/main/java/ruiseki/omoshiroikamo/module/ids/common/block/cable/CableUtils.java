...
import ruiseki.omoshiroikamo.module.ids.common.item.CableGrid;
import ruiseki.omoshiroikamo.module.ids.common.item.CableNetworkRegistry;

public class CableUtils {

    private static final Set<ICable> rebuildQueue = new HashSet<>();

    private CableUtils() {}

    public static void scheduleRebuild(ICable cable) {
        rebuildQueue.add(cable);
    }

    public static void processRebuildQueue() {
        if (rebuildQueue.isEmpty()) return;

        Set<ICable> processed = new HashSet<>();

        for (ICable cable : new HashSet<>(rebuildQueue)) {
            if (processed.contains(cable)) continue;

            CableCluster cluster = collectCluster(cable.getWorld(), cable);

            rebuildCluster(cluster);

            processed.addAll(cluster.cables);
        }
    ...
    private static void rebuildCluster(CableCluster cluster) {
        ...
        for (Map.Entry<Class<? extends ICableNode>, AbstractCableNetwork<?>> entry : newNetworks.entrySet()) {
            AbstractCableNetwork<?> net = entry.getValue();
            net.setGrid(grid);
            grid.addNetwork(entry.getKey(), net);
        }
