...
public class ItemImporter extends AbstractWriterPart implements IItemPart {
    ...
    private ItemStackPredicate buildPredicate(ItemStack stack) {
        ...
        if (nbt) {
            predicate = ItemStackPredicate.matches(stack);
        } else {
            predicate = ItemStackPredicate.matches(new ItemStack(stack.getItem(), 1, stack.getItemDamage()));
        }

        if (stackSize) {
            predicate = predicate.withStackSize(transferLimit, Integer.MAX_VALUE);
        }
