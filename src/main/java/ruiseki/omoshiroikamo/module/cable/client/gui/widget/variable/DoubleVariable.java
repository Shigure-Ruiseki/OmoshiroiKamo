...
public class DoubleVariable extends ParentWidget<DoubleVariable> {
    ...
    public DoubleVariable(ProgrammerPanel panel) {
        ...
        col.child(new TextWidget<>(IKey.str("Double config")));
            ...
            .onUpdateListener(widget -> {
                if (!widget.getText()
                    ...
                    panel.syncHandler.syncToServer(
                        ProgrammerSH.SET_DOUBLE_LITERAL,
                        buffer -> buffer.writeDouble(widget.parse(widget.getText())));
                }
            });
