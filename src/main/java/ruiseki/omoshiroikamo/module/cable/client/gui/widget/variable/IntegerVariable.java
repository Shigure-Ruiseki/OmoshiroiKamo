...
public class IntegerVariable extends ParentWidget<IntegerVariable> {
    ...
    public IntegerVariable(ProgrammerPanel panel) {
        ...
        col.child(new TextWidget<>(IKey.str("Integer config")));
            ...
            .onUpdateListener(widget -> {
                if (!widget.getText()
                    ...
                    panel.syncHandler.syncToServer(
                        ProgrammerSH.SET_INT_LITERAL,
                        buffer -> buffer.writeInt((int) widget.parse(widget.getText())));
                }
            });
