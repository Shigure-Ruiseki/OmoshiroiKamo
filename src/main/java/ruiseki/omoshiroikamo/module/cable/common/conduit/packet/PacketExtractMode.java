...
import ruiseki.omoshiroikamo.module.cable.common.conduit.IExtractor;
...
public class PacketExtractMode extends AbstractConduitPacket<IExtractor>
    ...
    public IMessage onMessage(PacketExtractMode message, MessageContext ctx) {
        ...
        message.getTileCasted(ctx)
            .setExtractionRedstoneMode(message.mode, message.dir);
        message.getTileCasted(ctx)
            .setExtractionSignalColor(message.dir, message.color);
        message.getWorld(ctx)
            .markBlockForUpdate(message.x, message.y, message.z);
        return null;
    }
}
