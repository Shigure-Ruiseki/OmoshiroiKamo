package ruiseki.omoshiroikamo.module.multiblock.common.block.modifier;

import java.util.ArrayList;
import java.util.List;

import ruiseki.okcore.helper.LangHelpers;
import ruiseki.omoshiroikamo.Reference;
import ruiseki.omoshiroikamo.api.enums.ModObject;
import ruiseki.omoshiroikamo.api.multiblock.AttributeEnergyCostFixed;
import ruiseki.omoshiroikamo.api.multiblock.IModifierAttribute;
import ruiseki.omoshiroikamo.config.backport.multiblock.QuantumBeaconConfig;
import ruiseki.omoshiroikamo.module.multiblock.common.init.ModifierAttribute;

public class BlockModifierWaterBreathing extends BlockModifier {

    protected BlockModifierWaterBreathing() {
        super(ModObject.MODIFIER_WATER_BREATHING.name, "water_breathing");
        setTextureName(Reference.PREFIX_MOD + "multiblock/modifier_water_breathing");
    }

    public static BlockModifierWaterBreathing create() {
        return new BlockModifierWaterBreathing();
    }

    @Override
    public void addAttributes(List<IModifierAttribute> list) {
        int energyCost = QuantumBeaconConfig.modifierEnergyCost.waterBreathing;
        list.add(ModifierAttribute.P_WATER_BREATHING.getAttribute());
        list.add(new AttributeEnergyCostFixed(energyCost));
    }

    @Override
    public List<String> getTooltipLines() {
        int energyCost = QuantumBeaconConfig.modifierEnergyCost.waterBreathing;
        List<String> list = new ArrayList<>();
        list.add(LangHelpers.localize("tooltip.modifier.water_breathing.effect"));
        list.add(String.format(LangHelpers.localize("tooltip.modifier.beacon.energy"), energyCost));
        return list;
    }
}
