 ...
 * Supports color blending through stained glass and colored lenses.
 */
@SideOnly(Side.CLIENT)
public class QuantumBeaconTESR extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity te, double x, double y, double z, float partialTicks) {
        TEQuantumBeacon beacon = (TEQuantumBeacon) te;
        if (beacon.isFormed()) {
            renderBeam(beacon, x, y, z, partialTicks);
        }
    }

    @SideOnly(Side.CLIENT)
    private void renderBeam(TEQuantumBeacon beacon, double x, double y, double z, float partialTicks) {
        float beamProgress = beacon.getBeamProgress();
    ...
    private void renderBeamSegment(Tessellator tessellator, double x, double y, double z, float worldTime,
        ...
        double startY = y + 1 + yOffset; // Start from top of beacon block
        double endY = startY + segmentHeight; // Go upward

        double d14 = -1.0D + textureOffset;
        double d15 = segmentHeight * (0.5D / beamWidth) + d14;

        // Inner beam (solid core)
