...
public class StorageSH extends SyncHandler {
    ...
    public StorageSH(PlayerMainInvWrapper playerInv, StorageWrapper wrapper) {
    ...
    public void setSortType(PacketBuffer buf) {
        wrapper.sortType = NetworkUtils.readEnumValue(buf, SortType.class);
    }

    public void sortInventory(PacketBuffer buf) throws IOException {
        for (int i = 0; i < wrapper.storageSlots; i++) {
            wrapper.storageItemStackHandler.setStackInSlot(i, buf.readItemStackFromBuffer());
        }
    }
