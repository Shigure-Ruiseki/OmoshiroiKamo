...
public class ItemPortRenderer implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack stack, Object... data) {
        ...
        } else if (block instanceof BlockMachineCasing casing) {
            // ===== MACHINE CASING =====
            renderMachineCasing(casing, meta, type);
        } else {
            // ===== BASE BLOCK (PORTS) =====
    ...
    /**
     * Renders the Machine Casing with tinted base and optional overlay.
     */
    private void renderMachineCasing(BlockMachineCasing casing, int meta, ItemRenderType type) {
        Tessellator tess = TessellatorManager.get();
        ...
        // Get base texture (tier-specific)
        IIcon baseIcon = casing.getIcon(0, meta);
        if (baseIcon == null) {
            // Fallback to a default if icon is missing
            return;
        }
        ...
        // Disable lighting for proper tint
        GL11.glDisable(GL11.GL_LIGHTING);
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        ...
        // Layer 2: Overlay (if design is not plain)
        String designName = casing.getDesignName();
        if (designName != null && !designName.isEmpty() && !designName.equals("plain")) {
            ...
            // IIcon overlayIcon = casing.getOverlayIcon(designName);
            // if (overlayIcon != null) {
            // tess.startDrawingQuads();
            // tess.setColorOpaque_F(1.0f, 1.0f, 1.0f); // No tint on overlay
            // for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            // tess.setNormal(dir.offsetX, dir.offsetY, dir.offsetZ);
            // RenderHelpers.renderFaceCorrected(tess, dir, 0, 0, 0, overlayIcon, 0.001f, Rotation.NORMAL, Flip.NONE);
            // }
            // tess.draw();
            // }
        }
