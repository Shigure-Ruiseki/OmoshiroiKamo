...
public class WorldHelpers {
    ...
    public static void setBiome(World world, BlockPos pos, BiomeGenBase biome) {
        ...
        if (chunk != null) {
            ...
            world.getChunkProvider()
                .provideChunk(chunk.xPosition, chunk.zPosition);
            ...
            Logger.warn("Tried changing biome at non-existing chunk for position " + pos);
        }
    }
     ...
     */
    public static <T> T foldArea(World world, int[] areaMin, int[] areaMax, BlockPos blockPos,
        WorldFoldingFunction<T, T> folder, T value) {
        int x = blockPos.getX();
        int y = blockPos.getY();
        int z = blockPos.getZ();
