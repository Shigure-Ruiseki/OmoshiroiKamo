...
public class ModCompatLoader implements IInitListener {
    ...
    public boolean shouldLoadModCompat(IModCompat modCompat) {
        return isModLoaded(modCompat) && isEnabled(modCompat, modCompat.getModID())
            && isNotCrashed(modCompat.getModID());
    }
    ...
    private boolean isModLoaded(IModCompat modCompat) {
        return LibResources.MOD_VANILLA.equals(modCompat.getModID()) || Loader.isModLoaded(modCompat.getModID());
    }

    private boolean isEnabled(IExternalCompat compat, String id) {
        // Configuration config = mod.getConfigHandler()
        // .getConfig();
