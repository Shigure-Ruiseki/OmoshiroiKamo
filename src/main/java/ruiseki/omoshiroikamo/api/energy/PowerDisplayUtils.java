...
public class PowerDisplayUtils {
    ...
    public static String getStoredEnergyString(ItemStack item) {
        ...
        return LibMisc.LANG.localize("item.tooltip.power") + " "
            + PowerDisplayUtils.formatPower(ci.getEnergyStored(item))
            + "/"
            + PowerDisplayUtils.formatPower(ci.getMaxEnergyStored(item))
            + " "
            + PowerDisplayUtils.abrevation();
    }
    ...
    public static String formatStoredPower(int amount, int capacity) {
        return formatPower(amount) + "/" + formatPower(capacity) + " " + PowerDisplayUtils.abrevation();
    }
    ...
    public static int parsePower(GuiTextField tf) {
        ...
        try {
            Integer power = PowerDisplayUtils.parsePower(txt);
            if (power == null) {
                return -1;
            }
