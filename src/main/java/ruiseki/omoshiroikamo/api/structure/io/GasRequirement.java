...
/**
 * Requirement for gas input/output.
 */
public class GasRequirement implements IStructureRequirement {

    private final String type;
    private final int min;
    private final int max;

    public GasRequirement(String type, int min, int max) {
        this.type = type;
        this.min = min;
        this.max = max;
    }
    ...
    public boolean matches(World world, int x, int y, int z) {
        ...
        if (te instanceof IModularPort) {
            ...
            return port.getPortType() == IPortType.Type.GAS;
        }
    ...
    public static IStructureRequirement fromJson(String type, JsonObject json) {
        ...
        return new GasRequirement(type, min, max);
    }
}
