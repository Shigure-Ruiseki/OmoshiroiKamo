...
/**
 * Requirement for vis input/output.
 */
public class VisRequirement implements IStructureRequirement {

    private final String type;
    private final int min;
    private final int max;

    public VisRequirement(String type, int min, int max) {
        this.type = type;
        this.min = min;
        this.max = max;
    }
    ...
    public boolean matches(World world, int x, int y, int z) {
        ...
        if (te instanceof IModularPort) {
            ...
            return port.getPortType() == IPortType.Type.VIS;
        }
    ...
    public static IStructureRequirement fromJson(String type, JsonObject json) {
        ...
        return new VisRequirement(type, min, max);
    }
}
