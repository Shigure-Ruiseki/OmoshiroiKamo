...
/**
 * Requirement for essentia input/output.
 */
public class EssentiaRequirement implements IStructureRequirement {

    private final String type;
    private final int min;
    private final int max;

    public EssentiaRequirement(String type, int min, int max) {
        this.type = type;
        this.min = min;
        this.max = max;
    }
    ...
    public boolean matches(World world, int x, int y, int z) {
        ...
        if (te instanceof IModularPort) {
            ...
            return port.getPortType() == IPortType.Type.ESSENTIA;
        }
    ...
    public static IStructureRequirement fromJson(String type, JsonObject json) {
        ...
        return new EssentiaRequirement(type, min, max);
    }
}
