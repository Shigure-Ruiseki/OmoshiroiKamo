...
- 20種類のテストケースでレシピシステムを網羅的に検証
  ...
  - **高度な機能・網羅性検証**: 
    - **Decorator**: `chance` (固定確率および統計的検証)、`bonus` (追加出力) の正常なパースと動作。
    - **Expression**: `map_range` と `nbt` を組み合わせた動的な数値計算（マシン状態による確率変動など）。
    - **Condition**: `biome` 指定による環境条件（特定のバイオームでのみ有効なレシピ）のパース。
    - **継承システム**: `parent` フィールドによる親レシピからのプロパティ継承とマージ。
    - **抽象レシピ**: `abstract: true` 設定によるテンプレート化（直接読み込まれないことの検証）。
    - **優先度とソート**: 
      - `priority` フィールドによる競合解決。
      - 同一優先度時の「入力アイテム数」による自動ソート順の検証。
- **テスト網羅性**: 以前の Ignore（検証待ち）を全て解消し、全26ケースでパース結果の詳細までアサーションを追加済み。
